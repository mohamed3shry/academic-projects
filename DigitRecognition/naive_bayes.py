...
logger = get_logger(log_dir="results", log_file="naive_bayes.log")
logger.info("Starting Gaussian Naive Bayes with 5-Fold CV and final training on full data...")


X, y, X_test, y_test = load_data( r"C:\Users\elwaled\Desktop\Handwritten Digit Recognition Using Machine Learning Techniques\data")
...
logger.info(f"CV Training completed in {elapsed:.2f}s ({elapsed/60:.2f} minutes)")
...
final_model.fit(X, y)
joblib.dump(final_model, "results/naive_bayes.pkl")
logger.info("Final model trained on full dataset saved to results/naive_bayes.pkl")
...
plt.ylabel("True")
plt.savefig("results/naive_bayes_confusion.png")
plt.show()
